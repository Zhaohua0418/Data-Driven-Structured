classdef Hinf_module_diag_structured
    ...
    methods
        function obj = Hinf_module_diag_structured(sys,pattern)
            obj.A = sys.A;
            obj.B = sys.B;
            obj.G = sys.G;
            obj.C = sys.C;
        ...
        function out = data_driven_Hinf(obj,info)
            SI = gen_si(info);
            cvx_begin sdp
                variables P(obj.nx,obj.nx) L(obj.nu,obj.nx) R(obj.nu+obj.nx,obj.nu+obj.nx) a b hinf_square
                minimize hinf_square
                subject to
