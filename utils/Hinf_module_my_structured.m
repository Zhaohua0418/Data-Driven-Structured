classdef Hinf_module_my_structured
    properties
        A,B,G,C,D,H,nx,nu,ny,nd,pattern
    end
    
    methods
        function obj = Hinf_module_my_structured(sys,pattern)
            obj.A = sys.A;
            obj.B = sys.B;
            obj.G = sys.G;
            obj.C = sys.C;
        ...
        function out = data_driven_Hinf(obj,info)
            ...
            while 1
                cvx_begin sdp
                    variables K(obj.nu,obj.nx) P(obj.nx,obj.nx) L(obj.nu,obj.nx) R(obj.nu+obj.nx,obj.nu+obj.nx) Y(obj.nx,obj.nx) hinf_square Z(obj.nx,obj.nx) tao(1,info.T) b
                    minimize hinf_square+lambda*trace(Z)
                    subject to
                            sum_over = 0;
                            for t = 1:info.T
